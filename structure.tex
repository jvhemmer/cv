...
\usepackage[letterpaper, left=1in, right=1in, top=1in, bottom=1in]{geometry} % Paper size and margins
...
\usepackage{needspace} % Keep grouped content together across page breaks
...
\titleformat{\section}
    {\Large\bfseries\color{slateblue}} % size, weight and color
    {} % no number
    {0em}
    {
        \storesectiontitle{#1}%
        \sbox{\sectiontitlebox}{#1}%
        \usebox{\sectiontitlebox}%
        \hspace{0.5em}%
        \raisebox{0.4ex}{
            \color{slateblue}\rule{\dimexpr\textwidth-\wd\sectiontitlebox-0.5em-5.962pt\relax}{1pt}
            }%
    }
...
% \dotfill but using a cdot instead of a regular dot. Credit: https://tex.stackexchange.com/questions/392739/how-to-fill-dots-between-two-textsdots-aligned-at-the-center-of-the-text
\makeatletter
\newcommand\cdotfill{%
...
\newif\ifskipnextcontinued
\skipnextcontinuedfalse

\newcommand{\skipcontinued}{%
	\global\skipnextcontinuedtrue
}

% \EveryShipout{%
%   \ifdim\pagetotal>\pagegoal
%     \ifskipnextcontinued
%       \global\skipnextcontinuedfalse
%     \else
%       \AddToHookNext{shipout/after}{%
%         \displaysectiontitle\, (continued)\cdotfill\par\bigskip
%       }%
%     \fi
%   \fi
% }

\newcommand{\showsectioncontinued}{%
    \displaysectiontitle\, (continued)\cdotfill\par
}

\newcommand{\continuednewpage}{%
	\AddToHookNext{shipout/after}{%
		\displaysectiontitle\, (continued)\cdotfill\par
	}%
	\newpage
...
\newcommand{\centervdotfill}{%
	\par
	\leaders\vbox to 1em{\vss\hbox to \textwidth{\hfil.\hfil}\vss}\vfill
	\nointerlineskip
}

\newcommand{\centervdottofooter}{%
	\par
	\begingroup
	\offinterlineskip
	\vspace{0.05in}
...
\newlength{\pubyearwidth}
\setlength{\pubyearwidth}{0.75in}
\newlength{\pubgutter}
\setlength{\pubgutter}{1em}
\newlength{\pubcontentwidth}
\setlength{\pubcontentwidth}{\dimexpr\textwidth-\pubyearwidth-\pubgutter\relax}
\newlength{\pubrowsep}
\setlength{\pubrowsep}{0.5\baselineskip}
\newsavebox{\pubcontentbox}
\newif\ifprintpubyear
\printpubyearfalse
\newif\iffirstpubinyear
\firstpubinyeartrue
\newcommand{\currentpubyear}{}

\newcommand{\publicationyear}[1]{%
	\par
	\vspace{\baselineskip}%
	\gdef\currentpubyear{#1}%
	\global\printpubyeartrue
	\global\firstpubinyeartrue
}

\newcommand{\publication}[5]{%
	\sbox{\pubcontentbox}{%
		\begin{minipage}[t]{\pubcontentwidth}%
			\iffirstpubinyear\else\vspace*{\pubrowsep}\fi
			#1\vspace{0.1em}\\
			#2\\
			\textit{#3}\\
			\href{#4}{#5}%
	...
	\begingroup
	\count255=\value{page}%
	\Needspace{\dimexpr\ht\pubcontentbox+\dp\pubcontentbox+2\baselineskip\relax}%
	\ifnum\value{page}>\count255
		\global\printpubyeartrue
	\fi
	\endgroup
	\noindent
	\begin{tabularx}{\textwidth}{@{}>{\columncolor{lightblue}[0pt][0pt]}p{\pubyearwidth}@{\hspace{\pubgutter}}p{\pubcontentwidth}@{}}
		\centering\textbf{\ifprintpubyear\currentpubyear\else\strut\fi} &
		\usebox{\pubcontentbox}
	\end{tabularx}%
	\par\nointerlineskip
	\global\printpubyearfalse
	\global\firstpubinyearfalse
}
